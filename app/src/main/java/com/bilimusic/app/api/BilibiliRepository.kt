    ...
    suspend fun getSubtitles(bvid: String): Result<List<LyricEntry>> = withContext(Dispatchers.IO) {
        try {
            ...
            for (i in 0 until subtitles.length()) {
                ...
                val lang = s.optString("lan_doc", "")
                if (lang.contains("中文") || lang.contains("chinese", ignoreCase = true)) {
                    selectedUrl = s.optString("subtitle_url", "")
                    break
                }
            }
            if (selectedUrl == null && subtitles.length() > 0) {
                selectedUrl = subtitles.getJSONObject(0).optString("subtitle_url", "")
            }
            if (selectedUrl.isNullOrEmpty()) return@withContext Result.failure(Exception("no subtitle url"))

            val subtitleContent = api.fetchText(selectedUrl)
            ...
            val entries = (0 until body.length()).map { i ->
                val b = body.getJSONObject(i)
                LyricEntry(
                    text = b.optString("content", ""),
                    startTimeMs = (b.optDouble("from", 0.0) * 1000).toLong(),
                    endTimeMs = (b.optDouble("to", 0.0) * 1000).toLong()
                )
            }
