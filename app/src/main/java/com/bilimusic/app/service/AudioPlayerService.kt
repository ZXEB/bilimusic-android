...
class AudioPlayerService : Service() {
    ...
    override fun onCreate() {
        ...
        scope.launch {
            PlayerManager.state.collectLatest { state ->
                ...
                val coverUrl = song?.cover.orEmpty().normalizeCoverUrl()
                if (song != null && coverUrl.isNotEmpty() && coverUrl != currentCoverUrl) {
                ...
                } else if (song == null || coverUrl.isEmpty()) {
    ...
    private suspend fun loadCoverBitmap(coverUrl: String): Bitmap? = withContext(Dispatchers.IO) {
        try {
            val normalizedUrl = coverUrl.normalizeCoverUrl()
    ...
    private fun updateMediaSession(
        ...
        if (song != null) {
            val coverUrl = song.cover.normalizeCoverUrl()
            val metaBuilder = MediaMetadataCompat.Builder()
                ...
                .putString(MediaMetadataCompat.METADATA_KEY_ART_URI, coverUrl)
                .putString(MediaMetadataCompat.METADATA_KEY_ALBUM_ART_URI, coverUrl)
                .putString(MediaMetadataCompat.METADATA_KEY_DISPLAY_ICON_URI, coverUrl)
